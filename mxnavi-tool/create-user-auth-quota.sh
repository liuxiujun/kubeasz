...
#用户在dashboard上可以获取集群内的namespace列表
kubectl get clusterrole list-namespaces
if [ $? != "0" ];then
  kubectl create -f - <<-EOC
kind: ClusterRole
...
kubectl create clusterrolebinding $USER-list-namespaces --clusterrole=list-namespaces --serviceaccount=$USER:default

#为指定namespace赋予read和shell权限
echo "==================================="
echo "为指定namespace赋予read和shell权限"
echo "==================================="
kubectl get ns
echo "=================================================="
read -p "输入赋予read和页面shell权限的namespace, 以空格分割：" NSTRING
OIFS=$IFS
IFS=" "
for i in $NSTRING
do
...
- apiGroups:
  ...
  - update
EOF
  kubectl -n $i create rolebinding $USER-read-shell --role=read-shell --serviceaccount=$USER:default
done
IFS=$OIFS

#为指定namespace赋予read权限

echo "==========================="
echo "为指定namespace赋予read权限"
echo "==========================="
echo "========================================="
read -p "输入赋予read权限的namespace, 以空格分割：" NSTRING
OIFS=$IFS
IFS=" "
for i in $NSTRING
do
...
- apiGroups:
  ...
  - list
EOF
  kubectl -n $i create rolebinding $USER-read-all --role=read-all --serviceaccount=$USER:default
done
IFS=$OIFS
